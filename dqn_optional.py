...
class DeepQNetwork:
   ...
   def __build_cnn1D(self): # this is 3 convolutional layers with 0.5 dropout, change it as you want to
        inputs = tf.keras.Input(shape=(self.state_size, self.history_length))
        ...
        x = layers.Conv1D(filters=16, kernel_size=2, strides=1, activation='relu', kernel_initializer=initializers.VarianceScaling(scale=2.))(x)
